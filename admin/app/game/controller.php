...
class GameController extends Controller{
    ...
    if(isset($_GET['desc'])){
		...
		$list = 8;
		$start = ($page-1)*$list;
		$sql = "SELECT COUNT(game_id) FROM `{$this->App->prefix()}game_option`";
	  ...
	  //  $sql .=" LEFT JOIN `{$this->App->prefix()}user` AS tb3 ON tb1.uid = tb3.user_id";
		...
		$sql = "SELECT * FROM `{$this->App->prefix()}game_option` LIMIT $start,$list";
		$gamelist = $this->App->find($sql);	 
		$this->set('gamelist',$this->App->find($sql));
	...
	function ads_info($type='ads_add',$id=0){
		...
		if($type=='ads_edit'){
		    if(empty($id)){
			  ...
			  $sql = "SELECT * FROM `{$this->App->prefix()}game_option` WHERE game_id='{$id}' LIMIT 1";
			  $rt = $this->App->findrow($sql);
	...
	function ajax_addads_info($data = array(),$game_id){
	    
        if( isset($game_id) ){
            
            $result = $this->App->update('game_option',$data,'game_id',$game_id);
        }
        else{
            
            $result = $this->App->insert('game_option',$data);
        }
	...
	function ajax_delads($ids){
		...
		$result = $this->App->delete('game_option','game_id',$ids);
// 		$arr = explode('+',$ids);
...
// 		$this->action('system','add_admin_log','删除广告：ID为'.implode(',',$arr));
        ...
        $this->action('system','add_admin_log','删除题目：ID为'.$ids);
	}
