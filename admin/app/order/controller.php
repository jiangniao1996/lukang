...
// 		$zlist = ceil($end/$list);
// 		$sql = "SELECT tb1.*,tb2.goods_name FROM `{$this->App->prefix()}goods_order_info` AS tb1 LEFT JOIN `{$this->App->prefix()}order_info` AS tb2 ON tb1.order_id=tb2.order_id WHERE tb1.pay_status = '1' ORDER BY tb1.order_id DESC LIMIT $start,$end";
		...
		if(isset($data['state'])){
			$state = $data['state'];
			$comd[] = $this->select_statue($state);
		}
		if (isset($data['order_id'])&&!empty($data['order_id'])) {
			$order_id = $data['order_id'];
			$comd[] = "order_id = " . $order_id;
		}
		if(isset($data['time1'])&&!empty($data['time1']) && empty($data['time2'])){
			
			$time1 = strtotime($data['time1']);
		    $t = strtotime($data['time1'])+24*60*60;
		    $comd[] = $time1 ."<= TIMESTAMP". "&&TIMESTAMP < " .$t;
		}
		if(isset($data['time2'])&&!empty($data['time2']) && empty($data['time1'])){
			$time2 = strtotime($data['time2']);
		    $comd[] = "TIMESTAMP <=" . $time2;
		}
		if(isset($data['time1']) && !empty($data['time1']) && isset($data['time2']) &&!empty($data['time2'])){
			$time1 = strtotime($data['time1']);
		    $time2 = strtotime($data['time2'])+24*60*60;
		    
		    $comd[] = " TIMESTAMP >= ".$time1." && TIMESTAMP <= " . $time2;
		}
		if(isset($data['delivery_name'])&&!empty($data['delivery_name'])) {
			$delivery_name = $data['delivery_name'];
			$comd[] = "delivery_name LIKE '%".trim($delivery_name)."%'";
		}
		if(isset($data['user_id'])&&!empty($data['user_id'])){
			$user_id = $data['user_id'];
			$comd[] = "user_id = " . $user_id;
		}
		...
		// }
		$w = str_replace("\\", "", $w);
		$sql = "SELECT * FROM `{$this->App->prefix()}order_info` $w $orderby LIMIT $start,$end";
...
// 		  header('Content-Type:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		  ...
		  if(!empty($rt)){
		      $str1 = '<table border="1" cellspacing="0" ccellpadding="0">';
		      $str1 .= '<tr>
                        <td style="text-align:center;" width="120">订单号</td>
                        <td style="text-align:center;" width="200">用户id</td>
                        <td style="text-align:center;" width="300">收货地址</td>
                        <td style="text-align:center;" width="100">收货人电话</td>
                        <td style="text-align:center;" width="70">收货人</td>
                        <td style="text-align:center;" width="200">礼物名称</td>
                        <td style="text-align:center;" width="120">下单时间</td>
                        <td style="text-align:center;" width="70">订单状态</td>
                        </tr>';
		      ...
		      foreach( $rt as $row ){
		          ...
		          $giftname = $this->get_giftname($row['gift_id']);
		          $TIMESTAMP = date('Y-m-d H:i:s', $row['TIMESTAMP']);
		          if($row['state']==1) $state = "已发货";
		          $str1 .= '<tr><td>'.$order_id.'</td><td>'.$user_id.'</td><td>'.$delivery_address.'</td><td>'.$delivery_phone.'</td><td>'.$delivery_name.'</td><td>'.$giftname.'</td><td>'.$TIMESTAMP.'</td><td>'.$state.'</td></tr>';
	...
	function order_print(){
			...
			//修改以打印状态
			$sql = "UPDATE `{$this->App->prefix()}order_info` SET is_print = '1' WHERE order_id IN(".implode(',',$ids_rt).")";
			$this->App->query($sql);

			 $sql = "SELECT * FROM {$this->App->prefix()}order_info WHERE order_id IN(".implode(',',$ids_rt).") ORDER BY TIMESTAMP DESC";
			 $orderlist = $this->App->find($sql);

			 if(!empty($orderlist)){
				 foreach($orderlist as $k => $row){
                    $orderlist[$k]['gift'] = $this->get_giftinfo($row['gift_id']);
	...
	function order_list(){
            ...
            if(isset($_GET['state']) || !empty($_GET['state'])){
                    $st = $this->select_statue($_GET['state']);
                    !empty($st)? $comd[] = $st : "";
            }
            if(isset($_GET['order_id'])&&!empty($_GET['order_id']))
                    $comd[] = "order_id LIKE '%".trim($_GET['order_id'])."%'";
			/********************************  look添加 日期查询 开始      ******************************/	
			if(isset($_GET['time1'])&&!empty($_GET['time1']) && empty($_GET['time2'])){
					$t = strtotime($_GET['time1'])+24*60*60 ;
                    $comd[] = "TIMESTAMP >= ". strtotime($_GET['time1']) ."&&TIMESTAMP < " .$t;
					}
			if(isset($_GET['time2'])&&!empty($_GET['time2']) && empty($_GET['time1'])){
                    $comd[] = "TIMESTAMP <= ". strtotime($_GET['time2']);
					}
			if(isset($_GET['time1'])&&!empty($_GET['time1']) &&isset($_GET['time2'])&& !empty($_GET['time2'])){
					$t = strtotime($_GET['time2'])+24*60*60 ;
                    $comd[] = "TIMESTAMP >= ". strtotime($_GET['time1']) ."&&TIMESTAMP < " .$t;
			}
							
            if(isset($_GET['delivery_name'])&&!empty($_GET['delivery_name']))
//                     $comd[] = "consignee LIKE '%".trim($_GET['consignee'])."%'";
                    $comd[] = "delivery_name LIKE '%".trim($_GET['delivery_name'])."%'";
			
            if(isset($_GET['user_id'])&&!empty($_GET['user_id']))
                    $comd[] = "user_id = " . $_GET['user_id'];
            ...
            $w = ""; 
                ...
                /**
                 * 在原来的系统中，当选择搜索id为1的用户订单后，显示订单，但点击如订单号，下单时间时，系统并不会将id=1的用户订单按正序或倒序排列，
                 * 只会跳回原来的总订单。因此加传一个$_GET['w']变量，此为一并传过来的搜索条件，如果有此搜索条件，则将此值覆盖之前的$w。相当于保留
                 * 了where条件。此时，我们就可以在用户id=1的订单下进行排序操作而不会跳回总订单了。
                 */
                if(!empty($_GET['w'])){
				...
				$sql = "select count(order_id) from `{$this->App->prefix()}order_info` $w";
				...
				$sql = "select order_id, user_id, delivery_address, delivery_phone, delivery_name, delivery_tips, gift_id, state, sn_id, shipping_id_true, TIMESTAMP, is_print from {$this->App->prefix()}order_info $w group by order_id $orderby limit $start,$list";
				$sql = str_replace("\\", "", $sql);
				$rt = $this->App->find($sql);
				$orderlist = array();
				foreach ($rt as $row){
					...
					//赋值收货人名字
					$sql = "select delivery_name from {$this->App->prefix()}order_info where order_id = $row[order_id]";
					$nickname = $this->App->findrow($sql);
					if( $row['shipping_id_true'] > 0){
						$sql = "select shipping_name from {$this->App->prefix()}shipping_name where shipping_id = $row[shipping_id_true]";
					...
					$orderlist[] = array(
						'order_id'=>$row['order_id'],
						'user_id' =>$row['user_id'],
						'TIMESTAMP'=>(!empty($row['TIMESTAMP'])? date('Y-m-d H:i:s',$row['TIMESTAMP']) : '未知'),
                        'gift_name' => $this->get_giftname($row['gift_id']),
						'state'=>$row['state'],
						'shipping_id_true'=>$row['shipping_id_true'],
						'shipping_name'=>$shipping_name,
						'delivery_name'=>$nickname["delivery_name"],
						'sn_id' => $row['sn_id'],
...
//                         'add_time'=>(!empty($row['add_time'])? date('Y-m-d H:i:s',$row['add_time']) : '未知'),
...
//         function get_status($oid=0,$pid=0,$sid=0){ //分别为：订单 支付 发货状态
        function get_status($sid=0){ //分别为：订单 支付 发货状态
            $str = '';
//             switch($oid){
//                 case '0':
//                     $str .= '未确认,';
...
// 		    var_dump($data);
		    if($state == 1){
		        ...
		        $this->App->update('order_info',array('state' => $state),'order_id',$order_id);
...
/***************  look添加 结束    *************************************************************************/		
		...
		function get_giftinfo($ids){
		    ...
		    $name_arr = array_count_values($name_arr);
		    
		    $name = array_keys($name_arr);
		    $num = array_values($name_arr);
            $length = count($name);
		...
		function get_giftname($str){
		      $names = explode(";",$str);
		      array_pop($names);
		      $length = count($names);
		      $name_arr = array();
		      for( $i=0; $i<=$length-1; $i++){
		          $sql = "select gift_name from gz_gift where gift_id = $names[$i]";
		          $res = $this->App->findrow($sql);
		          $name_arr[$i] = $res['gift_name'];
		      }
