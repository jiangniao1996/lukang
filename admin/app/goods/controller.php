...
// 	        var_dump($_GET);
            ...
            if(isset($_GET['desc'])){
                      ...
                      $orderby = ' ORDER BY tb1.`gift_id` ASC';
            }
            //分页
...
//                     $comd[] = 'tb1.uid='.intval($_GET['uid']);
			...
			*/
                if(isset($_GET['business_id'])&&intval($_GET['business_id'])>0)
                    $comd[] = 'tb1.business_id='.intval($_GET['business_id']);
				/*
...
//                     $comd[] = "(tb1.goods_name LIKE '%".trim($_GET['keyword'])."%' OR tb1.goods_sn LIKE '%".trim($_GET['keyword'])."%')";
				       $comd[] = "(tb1.gift_name LIKE '%".trim($_GET['keyword'])."%')";
				}
                if(!empty($comd)){
...
//             $sql .=" $w $orderby LIMIT $start,$list";// echo $sql;
            ...
            $sql = "SELECT gift_stock,gift_id,gift_name,tb1.business_id,image_url,gift_value,gift_coin,timestamp,business_name FROM `{$this->App->prefix()}gift` AS tb1 INNER JOIN `{$this->App->prefix()}business` AS tb2 ON tb1.business_id = tb2.business_id $w $orderby LIMIT $start,$list";
            $rt = $this->App->find($sql);
		  ...
		  //商家列表
			$sql = " SELECT * FROM {$this->App->prefix()}business";
			$arr_business_name = $this->App->find($sql);
			$this->set('arr_business_name',$arr_business_name);
...
// 		$this->set('rd',$this->App->findrow($sql));
			...
			//显示礼物详细信息
			$sql = "SELECT gift_stock,gift_name,gift_coin,gift_value,image_url,tb2.business_name FROM `gz_gift` AS tb1 INNER JOIN gz_business AS tb2 ON tb1.business_id = tb2.business_id WHERE tb1.gift_id = $gid";
			$rt = $this->App->findrow($sql);
			$this->set('rt',$rt);
...
// 	    var_dump($id_arr);
        ...
        $this->App->delete('gift','gift_id',$id_arr);
        return true;
