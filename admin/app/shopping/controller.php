...
class ShoppingController extends Controller{
	...
	function ajax_shopping_op($data){
		...
		if($oid > 0 && $sid>0 && !empty($sn)){
			$ssn = $this->App->findvar("SELECT sn_id FROM `{$this->App->prefix()}order_info` WHERE order_id='$oid' LIMIT 1");
			if(!empty($ssn)&&$ssn!==$sn){
...
// 			}
			$this->App->update('order_info',array('sn_id'=>$sn,'shipping_id_true'=>$sid),'order_id',$oid);
