...
class CuxiaoController extends Controller{
	...
	function order_erweima($data=array()){
	    $ym = "order_erweima";

	    /**
	     * uid作为user表中的user_id
	     * gid作为order_info表中的order_id或货运单号。存疑
	     */
	    $uid = 1;
	    $gids = explode('-',$data['ids']);

	    
	    $sql = "SELECT user_id FROM {$this->App->prefix()}order_info WHERE order_id IN (".implode(',',$gids).")";
...
// 	    $thisurl = SITE_URL."m/product.php?id=".$gid."&tid=".$uid;
	    ...
	    $filename = SYS_PATH_PHOTOS.$ym.DS.$gid.DS.$uid.'.png';
	    ...
	    // 点的大小：1到10
	    $matrixPointSize = 3;
	    QRcode::png("http://m.kuaidi100.com/index_all.html?type=&postid="."$gid", $filename, $errorCorrectionLevel, $matrixPointSize, 2);
