...
if($_POST['action']){
	switch($_POST['action']){
		...
		case 'addads': //添加广告
			 ...
			 $data['game_time'] = $_POST['game_time'];
			 $data['game_frequency'] = $_POST['game_frequency'];
			 $data['success_score'] = $_POST['success_score'];
			 $data['basic_score'] = $_POST['basic_score'];
			 $data['success_gift_coin'] = $_POST['success_gift_coin'];
			 $data['sum_challenged'] = $_POST['sum_challenged'];
             
		     $app->action('game','ajax_addads_info',$data,$game_id);
			break;
		case 'deladstag'://删除广告标签
			$app->action('ads','ajax_deladstag',$_POST['tids']);
			break;
		case 'delads': //删除广告
			$app->action('ads','ajax_delads',$_POST['game_id']);
			break;
		case 'activeop': //激活标签
			 $data['is_show'] = $_POST['active'];
			 $data['uptime'] = time(); 
...
switch($type){
	...
	case 'ads_add': //添加广告
	case 'ads_edit': //编辑广告
		$app->action('game','ads_info',$type,($_GET['id'] ?  $_GET['id'] : 0));
		break;
	$app->action('game','adslist');
	   break;
	}
